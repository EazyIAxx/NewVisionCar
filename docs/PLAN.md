...
- [ ] Trocar Stripe de modo teste para produção (chaves live + webhook live) — tentado e revertido; ver nota abaixo

**Nota:** Primeira tentativa de troca para modo live foi feita e revertida no mesmo dia: as chaves live, os 2 Price IDs live (conferidos via API — "NewVisionCar Basic" R$129,90 e "NewVision IA PRO" R$299,90, ambos `livemode: true`) e um novo webhook endpoint live foram configurados, mas a criação de sessão de checkout falhou com `"No valid payment method types for this Checkout Session"`. Causa raiz confirmada via `GET /v1/account`: `charges_enabled: false`, `card_payments: "pending"` — a verificação da conta Stripe (KYC/dados bancários) ainda não foi concluída pelo lado da Stripe, nada a ver com configuração nossa. Revertido para modo teste (checkout voltou a funcionar, confirmado via E2E). Para tentar de novo: esperar a Stripe aprovar a conta (verificar banner de pendências em dashboard.stripe.com), depois repetir a troca — chaves live, Price IDs live e o webhook endpoint live (`we_1UAJXi1evJ3KDioNkATD4kCr`) já estão prontos, só reconfigurar as env vars na Vercel e redeploy.
